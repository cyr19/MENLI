...
def bert_encode(model, x, attention_mask):
    ...
    with torch.no_grad():
        ...
        r = model(input_ids = x, token_type_ids = None, attention_mask = attention_mask).hidden_states
    #return x_encoded_layers
    return r

def tokenize_and_truncate(a, tokenizer):
    tokens = tokenizer.tokenize(a)
    if len(tokens)+2 > tokenizer.model_max_length:
        tokens = tokens[: tokenizer.model_max_length-2]
    assert len(tokens) + 2 <= tokenizer.model_max_length
    return tokens

def collate_idf(arr, tokenize, numericalize, idf_dict,
                pad="[PAD]", device='cuda:0'):
    tokens = [["[CLS]"]+tokenize_and_truncate(a, tokenize)+["[SEP]"] for a in arr]
...
def lm_perplexity(model, hyps, tokenizer, batch_size=1, device='cuda:0'):
    ...
    for batch_start in range(0, len(hyps), batch_size):
        ...
        #tokenize_input = tokenizer(batch_hyps[0], truncation=True)
