...
from transformers import AutoModel, GPT2Tokenizer, AutoTokenizer, AutoConfig, BertTokenizer
...
def get_model(model_type, num_layers, all_layers=None):
    ...
    if os.path.exists(model_dir):
        model = BertModel.from_pretrained(model_dir, cache_dir='.cache')
    else:
...
def get_tokenizer(model_type, use_fast=False):
    ...
    if os.path.exists(model_dir):
        #print('hhhhhhh')
        #print(os.path.exists(model_dir+'/bert_config.json'))
        #config = AutoConfig.from_pretrained(model_dir + '/bert_config.json')
        #tokenizer = AutoTokenizer.from_pretrained(model_dir, config=config, use_fast=use_fast)
        #tokenizer = AutoTokenizer.from_pretrained(model_dir, use_fast=use_fast)
        tokenizer = BertTokenizer.from_pretrained(model_dir, use_fast=use_fast)
    else:
        if model_type.startswith("scibert"):
