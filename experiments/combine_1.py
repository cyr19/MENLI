...
# MT
datasets_dict = {
        'ref': {'adv': ['paws_back_google', 'paws_ori_google', 'wmt20_google-de', 'xpaws-de'],
...
phenomena_dict = {
    'factuality': ['add', 'drop', 'name', 'neg', 'num', 'pron', 'word_JJ', 'word_NN', 'word_VB'],
    'fluency': ['typo', 'jumble', 'subject_verb_dis'],
    'all': ['add', 'drop', 'name', 'neg', 'num', 'pron', 'word_JJ', 'word_NN', 'word_VB',  'typo', 'jumble', 'subject_verb_dis']
}
...
def combine_nli(nli_metric, dataset, adv=True, agg='max', use_article=False, fluency=False, error_type='all'):
    suffix = '' if not use_article else "_use_article"
    if adv:
        if dataset != 'Rank19':
            f = '_fluency' if fluency else ''
            e_data_rh = load_metric_scores('../results/scores/adv_scores{}/{}_{}_rh{}.csv'.format(f, dataset, nli_metric, suffix), adv=True, prob='e')

            n_data_rh = load_metric_scores('../results/scores/adv_scores{}/{}_{}_rh{}.csv'.format(f, dataset, nli_metric, suffix), adv=True, prob='n')

            c_data_rh = load_metric_scores('../results/scores/adv_scores{}/{}_{}_rh{}.csv'.format(f, dataset, nli_metric, suffix), adv=True, prob='c')

            if not use_article:
                e_data_hr = load_metric_scores(
                    '../results/scores/adv_scores{}/{}_{}_hr{}.csv'.format(f, dataset, nli_metric, suffix), adv=True,
                    prob='e')
                n_data_hr = load_metric_scores(
                    '../results/scores/adv_scores{}/{}_{}_hr{}.csv'.format(f, dataset, nli_metric, suffix), adv=True,
                    prob='n')
                c_data_hr = load_metric_scores(
                    '../results/scores/adv_scores{}/{}_{}_hr{}.csv'.format(f, dataset, nli_metric, suffix), adv=True,
                    prob='c')
            r = defaultdict(lambda: defaultdict( lambda: defaultdict(dict)))
            for direction in ['rh', 'hr', 'avg']:
                if direction!='rh' and use_article:
                    continue
                if direction == 'rh':
                ...
                for error in e_data_rh.keys():
                    if error not in phenomena_dict[error_type]:
                        print('skip ', error)
                        continue
                    # -c
            ...
            # Rank19
            rh_data = load_metric_scores(f'../results/scores/adv_scores/Rank19_{nli_metric}_rh.pkl', adv=False)
            hr_data = load_metric_scores(f'../results/scores/adv_scores/Rank19_{nli_metric}_hr.pkl', adv=False)
            e_data_rh = rh_data['e']
            e_data_hr = hr_data['e']
            n_data_rh = rh_data['n']
            n_data_hr = hr_data['n']
        ...
        if 'wmt' in dataset:
            ...
            e_data_rh = load_metric_scores('../results/scores/{}_{}_scores/{}_rh_e_scores.pkl'.format(dataset, level, nli_metric), adv=False)
            e_data_hr = load_metric_scores('../results/scores/{}_{}_scores/{}_hr_e_scores.pkl'.format(dataset, level, nli_metric), adv=False)
            n_data_rh = load_metric_scores('../results/scores/{}_{}_scores/{}_rh_n_scores.pkl'.format(dataset, level, nli_metric), adv=False)
            n_data_hr = load_metric_scores('../results/scores/{}_{}_scores/{}_hr_n_scores.pkl'.format(dataset, level, nli_metric), adv=False)
            c_data_rh = load_metric_scores('../results/scores/{}_{}_scores/{}_rh_c_scores.pkl'.format(dataset, level, nli_metric), adv=False)
            c_data_hr = load_metric_scores('../results/scores/{}_{}_scores/{}_hr_c_scores.pkl'.format(dataset, level, nli_metric), adv=False)
            ...
            if dataset in ['summ', 'realsumm']:
                if dataset == 'realsumm' or use_article:
                    agg = 'None'
                e_data_rh = load_metric_scores(f"../results/scores/{dataset}_scores/{nli_metric}_rh_e_{agg}{suffix}.pkl", adv=False)
                n_data_rh = load_metric_scores(f"../results/scores/{dataset}_scores/{nli_metric}_rh_n_{agg}{suffix}.pkl", adv=False)
                c_data_rh = load_metric_scores(f"../results/scores/{dataset}_scores/{nli_metric}_rh_c_{agg}{suffix}.pkl", adv=False)
                if not use_article:
                    e_data_hr = load_metric_scores(
                        f"../results/scores/{dataset}_scores/{nli_metric}_hr_e_{agg}{suffix}.pkl", adv=False)
                    n_data_hr = load_metric_scores(
                        f"../results/scores/{dataset}_scores/{nli_metric}_hr_n_{agg}{suffix}.pkl", adv=False)
                    c_data_hr = load_metric_scores(
                        f"../results/scores/{dataset}_scores/{nli_metric}_hr_c_{agg}{suffix}.pkl", adv=False)
...
def combine_nli_and_metric(nli_metric, metric, dataset, method=add_scores, adv=True, norm='min_max', agg='max', use_article=False, direction='avg', strategy='e', fluency=False, error_type='all'):
    if adv:
        if dataset != 'Rank19':
            # load nli data
            ...
            suffix = '' if not use_article else '_use_article'
            f = '_fluency' if fluency else ''
            metric_data = load_metric_scores('../results/scores/adv_scores{}/{}_{}{}.csv'.format(f, dataset, metric, suffix), adv=adv)
            r = defaultdict(lambda: defaultdict(dict))
            ...
            nli_data = nli_data[direction][strategy]
            metric_data = load_metric_scores(f'../results/scores/adv_scores/Rank19_{metric}.pkl', adv=False)
        ...
        if 'wmt' in dataset:
            ...
            nli_data = nli_data[direction][strategy]
            metric_data = load_metric_scores('../results/scores/{}_{}_scores/{}_scores.pkl'.format(dataset, level_file, metric), adv=adv)
            ...
            if dataset in ['summ', 'realsumm']:
                ...
                if dataset == 'summ':
                    metric_data = load_metric_scores(f"../results/scores/{dataset}_scores/{metric}_{agg}{suffix}.pkl", adv=False)
                else:
                    metric_data = load_metric_scores(f"../results/scores/{dataset}_scores/{metric}_None{suffix}.pkl",
                                                     adv=False)
                nli_data = nli_data[direction][strategy]
                r = {}
...
def prepare_improvements_df_mt(setup='all', norm='min_max', suboptimal=False):
    setup_ori = setup
    if setup == 'all':
        setups = ['ref', 'src']
    else:
        setups = [setup]
    metric_r, nli_weight, dataset, type, improvement, ori, current, set_up, nli_metrics = [], [], [], [], [], [], [], [], []
    ...
    for setup in setups:
        ...
        if suboptimal:
            direction = 'hr' if setup == 'ref' else 'rh'
            strategy = 'e-c' if setup == 'ref' else '-c'
        else:
            direction = 'avg'
            strategy = 'e'
        for nli in nlis:
            for metric in metrics:
                ...
                for mt_dataset in mt_datasets:
                    if mt_dataset in datasets_dict[setup]['mt-sys'] and 'WMD' in metric:
                        continue
                    metric_data = combine_nli_and_metric(nli, metric, mt_dataset, adv=False, norm=norm, direction=direction, strategy=strategy)
                ...
                for adv_dataset in adv_datasets:
                    ...
                    for w, r in results.items():
                        ...
                        nli_metrics.append(nli)
                    # adequacy phenomena
                    metric_data = combine_nli_and_metric(nli, metric, adv_dataset, adv=True, norm=norm, error_type='factuality',
                                                         direction=direction, strategy=strategy)
                    results = evaluate(metric_data, adv_dataset, adv=True)
                    # original metric results
                    ori_r = np.mean(list(results[0.0].values()))
    ...
    print(data)
    #data.to_csv('../results/{}_improvements_data_mt_fixed.csv'.format(setup_ori), index=False)
    sub = '_suboptimal' if suboptimal else ''
    #data.to_csv(f'../results/suboptimal/{setup_ori}_improvements_data_mt_fixed{sub}.csv', index=False)
    return data
...
def prepare_improvement_table_data(setup='all', norm='min_max', level='seg'):
    ...
    print(data)
    data.to_csv('../results/tables/{}_improvement_table_data_{}_mt.csv'.format(setup_ori, level), index=False)
    return data


def generate_improvement_table_mt(setup):
    data_seg = pd.read_csv('../results/tables/{}_improvement_table_data_{}_mt.csv'.format(setup, 'seg'))
    data_sys = pd.read_csv('../results/tables/{}_improvement_table_data_{}_mt.csv'.format(setup, 'sys'))
    ...
    for metric in list(set(data_seg['metric'])):
        ...
        ori = mean_metric_data['ori'].values[0]
        final_data['ori.'].append("%.3f" % ori)
        for w in np.arange(0.1, 1, 0.1):
            w = float("%.1f" % w)
            current = mean_metric_data['combined-{}'.format(w)].values[0]
            #im = (current/ori-1) * 100
            im = (current-ori)/abs(ori) * 100
            final_data[w].append('{:.3f}({}{:.1f}%)'.format(current,
                                                           '+' if im > 0 else '',
                                                           im))
        ...
        for w in np.arange(0.1, 1, 0.1):
            ...
            if 'SentSim_new_WMD' not in metric:
                ...
                #im = (current/ori-1) * 100
                im = (current - ori) / abs(ori) * 100
                final_data[w].append('{:.3f}({}{:.1f}%)'.format(current, '+' if im > 0 else '', im))

            else:
                final_data[w].append('-')

    final_data = pd.DataFrame.from_dict(final_data)
    final_data.to_csv('../results/tables/final_improvement_{}_mt.csv'.format(setup), index=False)
    print(final_data)



def plot_phenomena_matrix(matrix, metrics, dataset, setup):
    phenomena = ['addition', 'omission', 'name error', "negation", 'number error', 'pronoun error',
                 'mis(adj)', 'mis(noun)', 'mis(verb)', 'typo', 'jumble', 'SVD']
    ms = []
    for metric in metrics:
    ...
    fig, ax = plt.subplots(figsize=(8, 7)) if setup=='ref' else plt.subplots(figsize=(8, 6))
    divider = make_axes_locatable(ax)
    cax = divider.append_axes('right', size='5%', pad=0.01)

    cmap = plt.cm.Reds
    im = ax.matshow(matrix, cmap=cmap, interpolation='nearest')
    ax.set_xticks([i+0.3 for i in range(len(matrix[0]))])
    ax.set_xticklabels(phenomena+['avg'], fontsize=10, rotation=45)
    ax.set_yticks(range(len(matrix)))
    ax.set_yticklabels(ms, fontsize=10)
    for (i, j), z in np.ndenumerate(matrix):
        ax.text(j, i, '{:0.1f}'.format(z), ha='center', va='center', fontsize=10)
    position = fig.add_axes([0.859, 0.11, 0.03, 0.772])
    cb = fig.colorbar(im,  cax=cax, orientation='vertical')
    plt.axis('off')
    plt.savefig('../results/plots/figure1_adv-matrix-{}-{}'.format(dataset,setup), dpi=300, bbox_inches='tight')
    plt.show()
    return ax


def plot_matrix(aa, directions, strategies, dataset, metric, adv):
    fig = plt.figure(figsize=(5, 3))
    cmap = plt.cm.OrRd if adv else plt.cm.PuBu
    print(cmap)
    ax = plt.imshow(aa, cmap=cmap, interpolation='nearest')
    plt.xticks(ticks=range(6),labels=strategies+['avg'], fontsize=14)
    plt.yticks(ticks=range(4),labels=directions+['avg'], fontsize=14)
    for (i, j), z in np.ndenumerate(aa):
        plt.text(j, i, '{:0.1f}'.format(z), ha='center', va='center', fontsize=14)
    #plt.title("{} on {}".format(metric, dataset), loc='center')
    plt.title(dataset,loc='center')
    position = fig.add_axes([0.859,0.11,0.03,0.772])
    cb = plt.colorbar(ax.colorbar, fraction=0.01, orientation='vertical',cax=position)
    #plt.savefig('../results/plot/{}-{}'.format(dataset, metric), dpi=300)
    plt.show()
    return ax
...
def plot_trade_off(nli1_data, nli2_data, title, setup, level):
    ...
    plt.tight_layout()
    plt.savefig('../results/plots/figure2_{}-{}-{}.png'.format(title, setup, level), dpi=300, bbox_inches='tight')
    plt.show()
...
def make_table_sum(direction='avg', strategy='e', agg='max', use_article=False, error_type='all'):
    ...
    for dataset in datasets_dict_sum['sum']+datasets_dict_sum['adv']:
        if dataset == 'Rank19' and not use_article:
            continue
        for i, metric in enumerate(sum_metrics_dict['ref']+['sentBLEU_default', 'Rouge_L'] if not use_article else sum_metrics_dict['src']):
            adv = True if dataset in datasets_dict_sum['adv'] else False

            metric_data = combine_nli_and_metric('NLI1Score_monolingual', metric, dataset, adv=adv, agg='mean', use_article=use_article,
                                                 direction=direction, strategy=strategy, error_type='all')
            r = evaluate(metric_data, dataset, adv=adv)
            if dataset != 'realsumm':
                if dataset in ['Rank19']:
                    final[dataset][metric] = r[0.0]
                elif dataset =='summ_google':
                    final[dataset][metric]['all'] = r[0.0]
                    final[dataset][metric]['all']['avg'] = np.mean(list(r[0.0].values()))
                    metric_data = combine_nli_and_metric('NLI1Score_monolingual', metric, dataset, adv=adv, agg='mean',
                                                         use_article=use_article,
                                                         direction=direction, strategy=strategy, error_type='factuality')
                    r = evaluate(metric_data, dataset, adv=adv)
                    final[dataset][metric]['adeq.'] = r[0.0]
                    final[dataset][metric]['adeq.']['avg'] = np.mean(list(r[0.0].values()))
                elif dataset == 'summ':
                    final[dataset][metric]['mean'] = r[0.0]
                    final[dataset][metric]['mean']['avg'] = np.mean(list(r[0.0].values()))
                    metric_data = combine_nli_and_metric('NLI1Score_monolingual', metric, dataset, adv=adv, agg='max',
                                                         use_article=use_article,
                                                         direction=direction, strategy=strategy)
                    r = evaluate(metric_data, dataset, adv=adv)
                    final[dataset][metric]['max'] = r[0.0]
                    final[dataset][metric]['max']['avg'] = np.mean(list(r[0.0].values()))
                else:
                    raise ValueError(f'No such dataset - {dataset} - {metric}')
            else:
                final[dataset][metric] = {'summary': r['summary'][0.0], 'system': r['system'][0.0]}
            if i == 0:
                for nli in nli_dict['ref']:
                    metric_data = combine_nli(nli, dataset, adv=adv, agg='mean', use_article=use_article, error_type='factuality')
                    r = evaluate(metric_data, dataset, adv=adv)
                    if dataset == 'summ':
                        final[dataset][nli]['mean'] = {'coherence': r['coherence'][direction][strategy],
                                               'consistency': r['consistency'][direction][strategy],
                                               'fluency': r['fluency'][direction][strategy],
                                               'relevance': r['relevance'][direction][strategy]}
                        final[dataset][nli]['mean']['avg'] = np.mean(list(final[dataset][nli]['mean'].values()))
                        metric_data = combine_nli(nli, dataset, adv=adv, agg='max', use_article=use_article)
                        r = evaluate(metric_data, dataset, adv=adv)
                        final[dataset][nli]['max'] = {'coherence': r['coherence'][direction][strategy],
                                                       'consistency': r['consistency'][direction][strategy],
                                                       'fluency': r['fluency'][direction][strategy],
                                                       'relevance': r['relevance'][direction][strategy]}
                        final[dataset][nli]['max']['avg'] = np.mean(list(final[dataset][nli]['max'].values()))
                    elif dataset == 'summ_google':
                        final[dataset][nli]['adeq.'] = r[direction][strategy]
                        final[dataset][nli]['adeq.']['avg'] = np.mean(list(r[direction][strategy].values()))
                        metric_data = combine_nli(nli, dataset, adv=adv, agg=agg, use_article=use_article,
                                                  error_type='all')
                        r = evaluate(metric_data, dataset, adv=adv)
                        final[dataset][nli]['all'] = r[direction][strategy]
                        final[dataset][nli]['all']['avg'] = np.mean(list(r[direction][strategy].values()))
                    elif dataset == 'realsumm':
...
def prepare_improvement_df_sum(setup ='ref', con=False):
    plt.style.use('seaborn-darkgrid')

    nli1, nli2 = defaultdict(dict), defaultdict(dict)
    if con:
        path = '../results/tables/all_improvement_data_sum_con.csv'
    else:
        path = '../results/tables/all_improvement_data_sum.csv'
    ...
    if setup == 'ref':
        # ref sum
        if con:
            combined_results = pd.read_csv(f'../results/tables/combined_sum/combined-on-sum-realsumm-ref-avg-e.csv')
        else:
            combined_results = pd.read_csv(f'../results/tables/combined_sum/combined-on-sum-realsumm-ref-hr-e-c.csv')
        ...
        if con:
            combined_results = pd.read_csv(f'../results/tables/combined_sum/combined-on-sum-summ-ref-avg-e.csv')
        else:
            combined_results = pd.read_csv(f'../results/tables/combined_sum/combined-on-sum-summ-ref-hr-e-c.csv')
        for metric in sum_metrics_dict['ref']:
            metric_data = combined_results[combined_results.metric == metric]
        ...
        # adv-all
        if con:
            combined_results = pd.read_csv(f'../results/tables/combined_sum/combined-on-adv-all-ref-avg-e.csv')
        else:
            combined_results = pd.read_csv(f'../results/tables/combined_sum/combined-on-adv-all-ref-hr-e-c.csv')
        ...
        # adv-fact
        if con:
            combined_results = pd.read_csv(f'../results/tables/combined_sum/combined-on-adv-factuality-ref-avg-e.csv')
        else:
            combined_results = pd.read_csv(f'../results/tables/combined_sum/combined-on-adv-factuality-ref-hr-e-c.csv')
        ...
        # src sum
        combined_results = pd.read_csv(f'../results/tables/combined_sum/combined-on-sum-realsumm-src-rh--c.csv')
        ...
        combined_results = pd.read_csv(f'../results/tables/combined_sum/combined-on-sum-summ-src-rh--c.csv')
        for metric in sum_metrics_dict['src']:
            metric_data = combined_results[combined_results.metric == metric]
        ...
        # adv-all
        combined_results = pd.read_csv(f'../results/tables/combined_sum/combined-on-adv-all-src-rh--c.csv')
        for metric in sum_metrics_dict['src']:
            metric_data = combined_results[(combined_results.metric == metric)]
        ...
        # adv-fac
        combined_results = pd.read_csv(f'../results/tables/combined_sum/combined-on-adv-factuality-src-rh--c.csv')
        for metric in sum_metrics_dict['src']:
            metric_data = combined_results[(combined_results.metric == metric)]
            summ_data = \
            metric_data[(metric_data.dataset == 'summ_google') & (metric_data.nli == 'NLI1Score_monolingual')][
                'avg'].values
            rank_data = \
            metric_data[(metric_data.dataset == 'Rank19') & (metric_data.nli == 'NLI1Score_monolingual')][
                'avg'].values

            for nli in nli_dict['ref']:
                for dataset in ['Rank19', 'summ_google']:
                    ori = metric_data[(metric_data.dataset == dataset) & (metric_data.nli == nli) & (
                                metric_data.nli_weight == 0.0)]['avg']
                    ori_nli = metric_data[(metric_data.dataset == dataset) & (metric_data.nli == nli) & (
                                metric_data.nli_weight == 1.0)]['avg']
                    assert len(ori) == 1
                    ori = ori.values[0]
                    ori_nli = ori_nli.values[0]
                    for w in np.arange(0.1, 1.0, 0.1):
                        w = float('%.1f' % w)
                        current = metric_data[(metric_data.dataset == dataset) & (metric_data.nli == nli) & (
                                    metric_data.nli_weight == w)]['avg']
                        assert len(current) == 1
                        current = current.values[0]
                        improvement = (current - ori) / abs(ori) * 100
                        improvement_nli = (current - ori_nli) / abs(ori_nli) * 100
...
def plot_sum_confidence(estimator='median', con=False):
    ...
    if estimator == 'median':
        ...
        es = estimator
    if con:
        data = pd.read_csv('../results/tables/improvement_table_data_sum_con_new_fixed_disco_4.csv')
    else:
        data = pd.read_csv('../results/tables/improvement_table_data_sum_new_fixed_disco_4.csv')

    data['type'] = [f"{t.split('-')[-1]}-Adv." if 'adv' in t else 'Sum.' for t in data['type']]
    ...
    if con:
        plt.savefig('../results/plot/confidence_interval_{}_all_sum_con_fixed_disco.png'.format(estimator), dpi=300,
                    bbox_inches='tight')
    else:
        plt.savefig('../results/plot/confidence_interval_{}_all_sum_fixed_disco.png'.format(estimator), dpi=300,
                    bbox_inches='tight')
    ...
    if con:
        plt.savefig('../results/plot/confidence_interval_{}_all_sum_nli_con_fixed_disco.png'.format(estimator), dpi=300,
                    bbox_inches='tight')
    else:
        plt.savefig('../results/plot/confidence_interval_{}_all_sum_nli_fixed_disco.png'.format(estimator), dpi=300,
                    bbox_inches='tight')

    plt.show()

def combined_sum(use_article=False, error_type='all'):
    ...
    for direction in ['rh', 'hr', 'avg']:
        ...
        for strategy in ['e', '-c', 'e-c']:
                ...
                for metric in metrics:
                    for nli in nli_dict['ref']:
                        if not use_article and dataset == 'Rank19':
                            continue
                        # all
                        metric_data = combine_nli_and_metric(nli, metric, dataset, adv=True, use_article=use_article, direction=direction, strategy=strategy, error_type=error_type)
            ...
            data.to_csv(f"../results/tables/combined_sum/combined-on-adv-{error_type}-{'ref' if not use_article else 'src'}-{direction}-{strategy}.csv", index=False)
            ...
            if error_type == 'all':
                ...
                data = pd.DataFrame(results)
                print(data)
                data.to_csv(f"../results/tables/combined_sum/combined-on-sum-{dataset}-{'ref' if not use_article else 'src'}-{direction}-{strategy}.csv", index=False)
